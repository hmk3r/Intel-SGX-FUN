...
char AES_AAD[BUFSIZ] = "lkyorovski-ENCLAVE-1";
...
/* returns sgx_status_t value (or 1337 if the counter has finished) */
    ...
    uint32_t *iv_ciphertext_size)
{
    ...
    /* 
    Decrypt ciphertext and assing to i only if
    some ciphertext was provided as input. Allows this function to be invoked
    without having received any ciphertext from the other app.
    */
    if (*iv_ciphertext_size)
    {
